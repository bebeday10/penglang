# Welcome to the PengLang Setup Guide

Welcome to the """friendly""" world of PengLang.  
It requires a decent bit of *Knowledge*™ on computers.  
However, because you're interested in coding PengLang, we'll assume you have *Knowledge*™.

# Step 1: You probably know but just in case...

Get Python. Search it up. Download it. Follow the steps.  

This is a requirement.

# Step 2: Get this.

> [!NOTE]
> You can now pip install the module.
> ```bash
> pip install penglang
> ```
> Skip to step 5, and read the notes:  
> - On step 6, do `import penglang.penglang`, `import penglang.modules.pengmath`, and `import penglang.modules.pengwindow`. For all, `import penglang`.

Go to Releases > Go to Latest > Download ZIP > File Explorer > Find that one specific ZIP > Extract it > You got it

# Step 3: Rename the folder

You don't need to anymore after v0.1.0.

# Step 4: Where to make projects
Anywhere, as long as you refer to the path of the import.

# Step 5: PengLang projects are Python projects

Use `.py` as the file extension. Please.

# Step 6: Hopefully you knew how to code Python

Here is some Python code.  
This is for the main PengLang language.

```python
import your.path.here.penglang_language.penglang_language.penglang as peng
```

`peng` can be whatever you want.

This is for the math module.

```python
import your.path.here.penglang_language.penglang_language.modules.pengmath as pengmath
```

`pengmath` can be whatever you want.

This is for the window module.

```python
import your.path.here.penglang_language.penglang_language.modules.pengwindow as pengwindow
```
`pengwindow` can be whatever you want.

Or, for all,

```python
import your.path.here.penglang_language.penglang_language as peng
```
`peng` can be whatever you want.

# Step 7: Python is a requirement now

You can code Python or you can code Python.

# Map

Here is a map of where you should put stuff (if not pip installed):

```
yourgamesomewherehere.py
penglang_language/
    penglang_language/
        modules/
            pengmath.py
            pengwindow.py
        penglang.py
        ___init__.py
    .gitignore
    LICENSE
    README.md
    setup.md [YOU ARE HERE]
```

# Conclusion

Every penguin hopes they can make the process faster.
